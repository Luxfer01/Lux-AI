...
function lux_questionGuide(tone, engagement) {
  ...
  function parseClientFactsFromLatestMessage(messageText) {
    ...
    const job = extractFirst(text, [
      ...
      /\bi'?m\s+(?:a\s+|an\s+)?([^\.,!?]{2,50})\s+(?:at\s+work|working)/i,
  ...
  async function autoLogLatestClientInfo(messageText) {
    try {
      ...
      if (!noteBox) {
        luxDebug('Member note box not found after wait');
        return false;
      }
      ...
      if (!changed && existingText.trim() === finalText.trim()) {
        luxDebug('Member note unchanged');
        return false;
      }
      if (lastHash && lastHash === newHash && existingText.trim() === finalText.trim()) {
        luxDebug('Member note hash unchanged');
        return false;
      }
      ...
      if (LUX_NOTE_AUTOSAVE) {
        ...
        luxDebug('Final member note', finalText, 'saveBtn', saveBtn);
        if (saveBtn) {
          forceClick(saveBtn);
          await sleep(120);
          forceClick(saveBtn);
        ...
        luxDebug('Final member note drafted', finalText);
